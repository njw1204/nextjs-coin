...
export default function Coin(counterProps: CounterProps) {
  ...
  const marketTradeBy = coin?.market.split('-')[0];
  const marketTradeWhat = coin?.market.split('-')[1];
      ...
      <div>
        {loading ? (
          ...
          <div className="coin-container">
            ...
            <h1 className="coin-trade-price">{coin?.trade_price.toLocaleString(undefined, { maximumFractionDigits: 20 })} {marketTradeBy}</h1>
            <br />
            <span>전일대비: </span>
            <span className="coin-change-container">
              <span>{((coin?.change_price ?? 0) >= 0) ? '+' : ''}{Math.round((coin?.change_rate ?? 0) * 1000000) / 10000}%</span>
              {' '}
              <span>({((coin?.change_price ?? 0) >= 0) ? '+' : ''}{coin?.change_price.toLocaleString(undefined, { maximumFractionDigits: 20 })})</span>
            </span>
          </div>
        )}
      </div>
